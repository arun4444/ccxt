...
const db = {
    ...
    fetchBalances: async function () {
        const balanceTable = (global._MOCK) ? 'mock_balances' : 'balances';
        const balances = await rdb.db('crypto_arb').table(balanceTable).run();
        return balances;
    },
    ...
    subscribeToBalanceUpdates: async function (handler) {
        ...
        try {
            var feed = await rdb.db('crypto_arb').table(balanceTable).changes().run();
    ...
    updateTrade: async function(trade){
        ...
        rdb.db('crypto_arb').table('trades').filter({id: trade.id}).replace(trade).run();
    },
    ...
    insertBalances: async function (objectToInsert) {
        await rdb.db('crypto_arb').table('balances').insert(objectToInsert, { conflict: "replace" }).run();
    },
